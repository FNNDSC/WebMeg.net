...
function displayOperations() {
	...
	if (MF.plotType=='raster') {
		...
		for (var i=0;i<MF.numCh;i++) {
			combo=absMax(MEG_data[i],1,1);
			maxValue=combo[0];
			// Normalize the array based on maximum value
			MEG_data[i]=normalizeArray(MEG_data[i],maxValue);
		}
...
// Function to switch the display to raster plot
function seriesPlot() {
	var MF=MEGFIFF;
	MF.plotType='raster';
	MEG_data=slice2d(finalData,MF.indDis);
...
function topoInit() {
	...
	if(isEmpty(MF.currentTime)) {alert('No time instant selected. Please select a time instant');return;}
		...
		if (canvas1.getContext) {
			...
			var mul=4;
			var heatmap=h337.create({"element":document.getElementById("heatmapArea"), "radius":40, "visible":true,"gradient": { 0: "rgb(0,0,255)", 0.25: "rgb(0,255,255)", 0.5: "rgb(0,255,0)", 0.75: "yellow", 1.0: "rgb(255,0,0)" },legend:{position:'br',title:' '}});
			normTopo();
			var chan=0;
			for (var i=0;i<MF.chNames.length;i++) {
				if (MF.goodCh[i]==1) {
					ctx.beginPath();
					var x=(MF.subLoc[i][0]*1000*mul)+250;
					var y=(MF.subLoc[i][1]*1000*mul)+250;
...
function normTopo() {
	...
	for (var i=0;i<MF.chNames.length;i++) {
		if (MF.goodCh[i]==1) {
			MF.topoVal[chan]=MEG_data[i][sampleNum];
			chan++;
		}
	}
...
function topoPlot() {
	var MF=MEGFIFF;
	$("#heatmapArea").empty();
	var heatmap=h337.create({"element":document.getElementById("heatmapArea"), "radius":40, "visible":true,"gradient": { 0: "rgb(0,0,255)", 0.25: "rgb(0,255,255)", 0.5: "rgb(0,255,0)", 0.75: "yellow", 1.0: "rgb(255,0,0)" },legend:{position:'br',title:' '}}), x, y;
	normTopo();
	var chan=0;
	var mul=4;
	for (var i=0;i<MF.chNames.length;i++) {
		if (MF.goodCh[i]==1) {
			x=(MF.subLoc[i][0]*1000*mul)+250;
			y=(MF.subLoc[i][1]*1000*mul)+250;
			try {
				heatmap.store.addDataPoint(x,y,MF.topoVal[chan]);
				chan++;
			}
...
function GFP(val) {
	var MF=MEGFIFF;
	var GFP=new Array(val[0].length),numCh=val.length,numS=val[0].length;
	for(var i=0;i<numS;i++) GFP[i]=0;
...
function plotCont() {
	...
	var MF=MEGFIFF;
	if (MF.avgDis == false) return;
	MF.plotType='raster';
	MF.disName=MF.fileName; 
	MF.avgDis=false;
	MEG_data=slice2d(finalData,MF.indDis);
	MEG_data=displayOperations();
	GFP(MEG_data);
