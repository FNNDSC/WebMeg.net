/* Function used to operate on data for display
 */
...
function topoPlot() {
    ...
    if (canvas.getContext) {
    	...
    	for (var i = 0;i < 74;i++) {
    		ctx.beginPath();
    		ctx.arc((data_info.info.chs[i].loc[0] * 1000 * mul)+ canvas.width/2.0,(data_info.info.chs[i].loc[1] * 1000 * mul) + canvas.width/2.0,3,0,Math.PI*2, true);
    		ctx.stroke();
    		ctx.fillText(MEGFIFF.chNames[i], (data_info.info.chs[i].loc[0] * 1000 * mul)+ canvas.width/2.0 - 5, (data_info.info.chs[i].loc[1] * 1000 * mul) + canvas.width/2.0 - 5);
    	}
    }
  }
